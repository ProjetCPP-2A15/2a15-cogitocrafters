...
void MainWindow::on_listtoaddpro_clicked()
{
    QSqlQuery query("SELECT cin FROM TABLE1");
       ...
       speech->say("cette page pour ajouter un produit.");
...
ui->tableView_17->setModel(article.afficher());
qDebug() << "Button clicked";
    speech->say("cette page pour modifier les produits.");
}
...
int nbvuu = index.sibling(index.row(), nbvu).data().toInt();
 ...
 qDebug() << "Button clicked";
      speech->say("cette page pour identifier les produits.");
...
{
qDebug() << "Button clicked";
       speech->say("Cette page affiche les tableaux sous forme d'un album.");
...
{
qDebug() << "Button clicked";
    speech->say("dans cette page tu peut suuprimer les produits .");
...
QModelIndexList selectedIndexes = ui->tableView_4->selectionModel()->selectedIndexes();
   if (selectedIndexes.isEmpty()) {
       qDebug() << "Button clicked";
                   speech->say("il faut sélectionner au moins un article à supprimer.");
...
{
    qDebug() << "Button clicked";
    speech->say("tri domaine descendant.");
...
{
    qDebug() << "Button clicked";
    speech->say("tri domaine ascendant.");
...
{
    qDebug() << "Button clicked";
    speech->say("tri date descendant.");
...
{
    qDebug() << "Button clicked";
    speech->say("tri type descendant.");
...
{
    qDebug() << "Button clicked";
    speech->say("tri dimension descendant.");
...
{
    qDebug() << "Button clicked";
    speech->say("tri dimension ascendant.");
...
{
    qDebug() << "Button clicked";
    speech->say("tri poids descendant.");
...
{
    qDebug() << "Button clicked";
    speech->say("tri poids ascendant.");
...
{
bool ok;
   int rating = QInputDialog::getInt(this, tr("Rating"),
                                     tr("Rating:"), 5, 0, 10, 1, &ok);
   if (ok) {
