...
QString evenement::getetat()const
{
    return etat ;
}
//setters
...
void evenement::setetat(QString etat)
{
     this ->etat=etat;
}
evenement::evenement()
...
evenement evenement::chercher(int id_event)
{
    ...
    if (query.exec())
    {
        if (query.next())
        {
            ...
            e.setetat(query.value("etat").toString());
...
bool evenement::ajouterEvenement()
{
    ...
    query.prepare("INSERT INTO EVENEMENT (ID_EVENT, NB_EMPLOYEE ,EVENT_NOUN,LOCATION ,DESCRIPTION ,START_D ,END_D,START_H,END_H,PRICE_TICK,RFID,etat) "
                          "VALUES (:id_event,:nb_employee ,:event_noun,:location ,:description ,:start_d ,:end_d,:start_h,:end_h,:price_tick,:RFID,:etat)");
...
bool evenement::updateEventState(const QString& state) {

        // Préparation de la requête SQL pour mettre à jour l'état de l'événement
        QSqlQuery query;
        query.prepare("UPDATE EVENEMENT SET etat = :state WHERE id_event = :id_event");
...
QString evenement::rechercherRFIDTime(QString rfid)
{
    QSqlQuery query;
        query.prepare("SELECT start_h FROM evenement WHERE RFID = ?");
        query.addBindValue(rfid);

        if (query.exec() && query.next())
        {
            return query.value("start_h").toTime().toString("hh:mm:ss");
        }
        else
        {
            qDebug() << "Query failed:" << query.lastError().text();
            // Si l'élément n'est pas trouvé ou s'il y a une erreur dans la requête, retournez une chaîne vide ou une indication d'absence.
            return QString(); // Retourne une chaîne vide par défaut.
        }
}
